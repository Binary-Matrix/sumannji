  ...
  return (
    <section className="border-t border-border/40 px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <p className="font-mono text-xs uppercase tracking-[0.3em] text-muted-foreground">03 · play</p>
        <h2 className="mt-2 font-display text-4xl sm:text-5xl">Hands on with this guys.</h2>
        <p className="mt-3 max-w-xl text-muted-foreground font-mono text-sm">
          tap an instrument · I will soon add my works in this, but for now it's just a demo for you guys.
        </p>
          ...
          {instruments.map((i) => {
            ...
            return (
              <button
                ...
                disabled={onCool}
                className="group glass tilt-3d rounded-xl p-5 text-left transition hover:-translate-y-1 hover:border-primary/50 hover:shadow-ember disabled:opacity-50"
              >
                <div className="text-3xl transition group-hover:scale-110">{i.emoji}</div>
                <p className="mt-3 font-display text-lg">{i.name}</p>
                <p className="font-mono text-[11px] text-muted-foreground">{i.note}</p>
        ...
        <div className="fixed inset-0 z-[300] grid place-items-center p-4">
          ...
          <div className="glass-strong relative z-10 w-full max-w-md overflow-hidden rounded-3xl">
            <div className="flex items-center justify-between border-b border-border/40 px-4 py-3">
              <p className="font-display text-base">🎼 Suman · bansuri reel</p>
              <button
            ...
            <div className="p-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block w-full rounded-full bg-ember px-4 py-2 text-center font-mono text-xs text-primary-foreground shadow-ember"
              >open on Instagram ↗</a>
            </div>
          </div>
        </div>
